...
const friendlyMessages: Record<string, string> = {
  ...
  "auth/weak-password": "Use a stronger password with at least 8 characters, a number, and mixed case letters.",
};

export function getAuthErrorMessage(error: unknown): string {
  if (error instanceof FirebaseError) {
    return friendlyMessages[error.code] ?? "Authentication could not be completed. Please try again.";
  }
